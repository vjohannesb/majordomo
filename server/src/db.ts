...
export async function saveUserSettings(settings: Partial<UserSettings> & { userId: string }): Promise<void> {
  if (!sql) throw new Error('Database not configured');
  ...
  // Use explicit values to avoid type issues with postgres template
  const updateChannel = settings.notificationChannel || null;

  await sql`
    INSERT INTO user_settings (user_id, notification_channel, slack_channel_id, webhook_secrets)
    VALUES (${settings.userId}, ${notificationChannel}, ${slackChannelId}, ${webhookSecrets})
    ON CONFLICT (user_id) DO UPDATE SET
      notification_channel = COALESCE(${updateChannel}, user_settings.notification_channel),
      slack_channel_id = COALESCE(${slackChannelId}, user_settings.slack_channel_id),
      webhook_secrets = COALESCE(${webhookSecrets}, user_settings.webhook_secrets),
      updated_at = NOW()
  `;
}
